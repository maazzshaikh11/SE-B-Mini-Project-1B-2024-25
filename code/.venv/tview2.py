...
import os
import tasks1  # Import tasks1 module
...
def on_view_attempt_click(org_name, section_name, task_name, username):
    print(f"[DEBUG] on_view_attempt_click: username = {username}")
    tasks1.open_task_file_adder(org_name, section_name, task_name, username)


def open_task_view_page(org_name, section_name, username):  # Accept username here
    print(f"[DEBUG] open_task_view_page called with username: {username}")
    ...
    columns = ("No", "Task Name", "Task Type", "File Name", "Submission Date", "Download", "View/Attempt")
    ...
    tree.column("Task Type", width=150)
    tree.column("File Name", width=300)
    tree.column("Submission Date", width=150)
    tree.column("Download", width=120, anchor="center")
    tree.column("View/Attempt", width=120, anchor="center")

    idx = 1
    for task_name, data in grouped_tasks.items():
        for i, file_name in enumerate(data["file_names"]):
            if i == 0:
                tree.insert("", "end", values=(idx, task_name, data["task_type"], file_name, data["submission_date"], "Download", "View/Attempt"), iid=f"{task_name}_{idx}")
            else:
                tree.insert("", "end", values=("", "", "", file_name, "", "", ""), iid=f"{task_name}_file_{i}")
        idx += 1
    ...
    def on_item_click(event):
        ...
        if selected_item:
            ...
            if values:
                ...
                if column == "#6":
                    if values[1]:
                        download_all_docs(values[1], table_name)
                elif column == "#7":
                    if values[1]:
                        on_view_attempt_click(org_name, section_name, values[1], username)  # Pass username
...
if __name__ == "__main__":
    open_task_view_page("example_org", "example_section", "john_doe")  # Example usage
