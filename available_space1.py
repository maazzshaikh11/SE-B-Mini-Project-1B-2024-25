...
update_parking_status("college")

# Load current parking data
parking_spaces = load_parking_data("college")

# Dictionary to store button widgets with college parking keys
buttons = {}
...
def redirect_to_time_cost(selected_space):
    ...
    # Save parking type to indicate we're using college
...
def show_parking_spaces():
    ...
    label_location = tk.Label(location_selection_frame, text="COLLEGE PARKING SPACES", font=("Arial", 30, "bold"), bg="#2C3E50", fg="white")
    label_location.pack(pady=30)

    # Create a frame for the grid of parking spaces
    grid_frame = tk.Frame(location_selection_frame, bg="#2C3E50")
    grid_frame.pack(pady=10)
    
    # Loop through each row and column of the parking lot grid
    for row_index in range(4):  # 4 rows (A, B, C, D)
        row_letter = row_letters[row_index]
        
        for col_index in range(5):  # 5 columns (1, 2, 3, 4, 5)
            # Generate position name (e.g., "A1", "B2", etc.)
            ...
            # Create button with position in text
            ...
            # Store button in the dictionary with position as key
            buttons[position] = space_button

    # Show the parking spaces page
...
root = tk.Tk()
root.title("College Parking Manager")
